...
```{note}
Два подряд идущих удаления помещают в буфер склеенный текст.
Чтобы создать для каждого удаленного текста свое место в буфере перед удалением следующего фрагмента текста выполните команду перемещения курсора.
```
...
1. Пройдитесь по всем фрагментам текста из буфера, прокручивая его.
1. В следующей командной строке поменяйте свободные аргументы местами:
   ```
   diff -u myproject/src/lib/file.cpp ~/Downloads/file_from_ivanov.cpp
   ```
   В начальной позиции курсор расположен в конце строки.
   Постарайтесь воспользоваться как можно меньшим количеством команд для этого.
   Сравните свой вариант с вариантом [отсюда](https://unix.stackexchange.com/questions/187600/how-do-i-swap-the-two-last-command-line-arguments).
1. В следующей командной строке поменяйте опции местами:
   ...
   В начальной позиции курсор расположен в конце строки.
   Постарайтесь воспользоваться как можно меньшим количеством команд для этого.
   Попробуйте воспользоваться командой `atl-t`.
1. В следующей командной строке замените аргумент `diff` на `add`:
   ```
   git diff ./tests/unittests/CMakeLists.txt
   ```
   В начальной позиции курсор расположен в конце строки.
   Постарайтесь воспользоваться как можно меньшим количеством команд для этого.
...
1. В [шпаргалке](https://catonmat.net/ftp/readline-emacs-editing-mode-cheat-sheet.txt) найдите неизвестную для себя команду.
   Продемонстрируйте ее на примере.
